...
export default function EmployeeLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // We rely on middleware or page layout to properly redirect, 
      // but we can push to root to let the server decide based on role
      window.location.href = "/admin"; // Will redirect to POS if Staff
    } catch (err) {
  ...
  return (
    <div className="relative flex min-h-[calc(100vh-3.5rem)] items-center justify-center p-4 overflow-hidden bg-zinc-50 dark:bg-zinc-950">
      <div className="absolute top-1/4 left-1/4 h-96 w-96 -translate-x-1/2 -translate-y-1/2 rounded-full bg-emerald-500/10 blur-[100px] pointer-events-none" />
      ...
      <div className="relative w-full max-w-md rounded-2xl border border-zinc-200/50 dark:border-zinc-800/50 bg-white/70 dark:bg-zinc-900/50 p-8 shadow-2xl backdrop-blur-xl">
        <div className="mb-8 flex flex-col items-center text-center">
          <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-xl bg-emerald-500 text-white shadow-lg shadow-emerald-500/20">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <button
            type="submit"
            disabled={loading}
            className="group relative inline-flex h-11 w-full items-center justify-center overflow-hidden rounded-lg bg-emerald-500 px-4 py-2 text-sm font-medium text-white transition-all hover:scale-[1.02] hover:bg-emerald-600 focus:outline-none active:scale-[0.98] shadow-md shadow-emerald-500/20 disabled:pointer-events-none disabled:opacity-50 mt-4"
          >
            {loading ? (
